"""
paper_trading/small_capital_strategy/paper_simulation_safety_v180.py
Safety flags for Paper Simulation & Performance Lab v1.8.0.
...
def get_safety_flags() -> Dict[str, Any]:
    """Return a copy of the SAFETY_FLAGS dict."""
    return dict(SAFETY_FLAGS)


def run_safety_audit() -> Dict[str, Any]:
    """Run safety audit and return result dict with all_safe bool and issues list."""
    issues: List[str] = []
...
def assert_safe() -> None:
    ...
    if not result["all_safe"]:
        raise RuntimeError(
            f"Safety audit failed: {result['issues']}"
        )
