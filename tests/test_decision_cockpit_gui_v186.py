...
def test_panel_version_186():
    assert PANEL_VERSION in ("1.8.6", "1.8.7")

def test_panel_title_contains_186():
    assert "1.8.6" in PANEL_TITLE or "1.8.7" in PANEL_TITLE

def test_panel_title_contains_decision_cockpit():
    assert "Decision Cockpit" in PANEL_TITLE or "Decision Report" in PANEL_TITLE
...
def test_render_overview_tab_version():
    assert render_overview_tab()["version"] in ("1.8.6", "1.8.7")
