...
def test_panel_version_195():
    assert PANEL_VERSION in ("1.9.5", "1.9.6")


def test_panel_title_contains_195():
    assert "1.9.5" in PANEL_TITLE or "1.9.6" in PANEL_TITLE


def test_panel_title_contains_review_or_approval():
    assert "Review" in PANEL_TITLE or "Approval" in PANEL_TITLE
...
def test_get_panel_info_panel_version():
    assert get_panel_info()["panel_version"] in ("1.9.5", "1.9.6")
