...
def test_panel_version_is_1_9_9():
    assert PANEL_VERSION in ("1.9.9",)


def test_panel_title_contains_1_9_9():
    assert "1.9.9" in PANEL_TITLE


def test_panel_title_contains_expected_keyword():
    assert any(kw in PANEL_TITLE for kw in ("Risk Report", "Position Sizing", "Portfolio"))
...
def test_get_panel_info_panel_version_is_1_9_9():
    assert get_panel_info()["panel_version"] in ("1.9.9",)
