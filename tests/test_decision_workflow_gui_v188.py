...
def test_panel_version_is_188():
    assert PANEL_VERSION in ("1.8.8", "1.8.9", "1.9.0")


def test_panel_title_contains_188():
    assert "1.8.8" in PANEL_TITLE or "1.8.9" in PANEL_TITLE or "1.9.0" in PANEL_TITLE


def test_panel_title_contains_workflow_runner():
    assert "Workflow Runner" in PANEL_TITLE or "Journal" in PANEL_TITLE or "Performance" in PANEL_TITLE
...
def test_render_decision_workflow_tab_version():
    data = render_decision_workflow_tab()
    assert data["version"] in ("1.8.8", "1.8.9", "1.9.0")
...
def test_get_panel_info_version():
    info = get_panel_info()
    assert info["panel_version"] in ("1.8.8", "1.8.9", "1.9.0")
