...
def test_panel_version_is_187():
    assert PANEL_VERSION in ("1.8.7", "1.8.8")


def test_panel_title_contains_187():
    assert "1.8.7" in PANEL_TITLE or "1.8.8" in PANEL_TITLE


def test_panel_title_contains_decision_report():
    assert "Decision Report" in PANEL_TITLE or "Workflow Runner" in PANEL_TITLE
...
def test_render_decision_report_tab_version():
    result = render_decision_report_tab()
    assert result["version"] in ("1.8.7", "1.8.8")
...
def test_get_panel_info_panel_version():
    info = get_panel_info()
    assert info["panel_version"] in ("1.8.7", "1.8.8")
